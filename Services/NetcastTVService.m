...
- (void) sendLGKeyCode:(LGKeyCode)keyCode success:(SuccessBlock)success failure:(FailureBlock)failure {
    NetcastTVKeyCode code = 0;
    if (keyCode >= LGKeyCodeNum0  && keyCode <= LGKeyCodeNum9) {
        code = keyCode - LGKeyCodeNum0 + NetcastTVKeyCodeNumber0;
    } else if (keyCode == LGKeyCodeDash) {
        code = NetcastTVKeyCodeDash;
    } else if (keyCode == LGKeyCodeEnter) {
        code = NetcastTVKeyCodeOK;
    }
    if (code != 0) {
        [self sendKeyCode:code success:success failure:failure];
    }
}
